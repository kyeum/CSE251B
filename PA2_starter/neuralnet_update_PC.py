...
class Layer():
    ...
    def updateweight(self, lr, l2_penalty, momentum, momentum_gamma):
        ...
        if (momentum) : 
            ...
            self.w -= lr * (self.d_w + momentum_gamma * self.pre_d_w) / 350
            self.b -= lr * (self.d_b + momentum_gamma * self.pre_d_b) / 350
...
class Neuralnetwork():
    ...
    def forward(self, x, targets=None):
        ...
        self.targets = self.targets.reshape((-1, self.output_size))
    ...
    def loss(self, logits, targets):
        ...
        # print("LOG:", logits[range(len(logits)), y_true] + epsilon)
        temp = logits[range(len(logits)), y_true] + epsilon
        if temp[0] < 0:
            print("AHHHHH")
            print(temp)
    ...
    def backward(self):
        ...
        '''
        self.num_batches += 1
        # divide by batch size and num classes
