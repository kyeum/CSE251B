...
from coco_dataset_Morris import CocoDataset, collate_fn
...
def get_coco_dataloader(img_ids_file_path, imgs_root_dir, annotation_file_path, coco_obj, vocabulary, config_data, transform=None):
    ...
    ann_ids = [coco_obj.imgToAnns[img_ids[i]][j]['id'] for i in range(0, len(img_ids)) for j in range(0, len(coco_obj.imgToAnns[img_ids[i]]))]
